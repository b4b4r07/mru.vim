...
if !exists('MRU_Auto_Remove_Deadfiles')
	let MRU_Auto_Remove_Deadfiles = 1
endif
...
function! s:MRU_Open_Window(...)
	...
	nnoremap <buffer> <silent> U :MRU<CR>
	nnoremap <buffer> <silent> Z :call <SID>Star('')<CR>
	" ==== Modify start by b4b4r07 {{{
	...
	" b4b4r07 {{{
	if g:MRU_Auto_Remove_Deadfiles
		let ridx = 0
		for file in l:m
			if !filereadable(file)
				call remove(l:m, ridx)
				continue
			endif
			let ridx += 1
		endfor
	...
	for idx in range(0, len(glist)-1)
		...
		let glist[idx] = printf("%s%-" . max .  "s | %-" . max_h . "s | %s" ,
					\ s:Star(glist[idx]), fnamemodify(glist[idx], ':t'), substitute(fnamemodify(glist[idx], ':p:h'), '^.*\/', '', ''), glist[idx])
	endfor
	silent! %delete _
	...
	let hlpattern = '"'.'|\zs[^|]*\ze|'.'"'
	exe 'syntax match MruPdirPath display ' . hlpattern
	if exists('g:MRU_hi_PdirPath')
		exe 'hi MruPdirPath ' . g:MRU_hi_PdirPath
	endif
...
" ==== Modify start {{{1
if !exists('g:MRU_hi_Fname')
	hi link MruFname      Title
endif
if !exists('g:MRU_hi_FnamePath')
	hi link MruFnamePath  Constant
endif
if !exists('g:MRU_hi_PdirPath')
	hi link MruPdirPath Directory
endif
...
"command! MruStar call s:Star()
function! s:Star(star)

	let s:MRU_starfile_url = expand('~/.mru_starfile')
	let s:MRU_starfile = readfile(s:MRU_starfile_url)

	if empty(a:star)
		setlocal modifiable
		if getline('.') =~# '^ '
			call setline('.', substitute(getline('.'), '^ ', '*', 'g'))
			call add(s:MRU_starfile, substitute(getline('.'), '^.*| ', '', 'g'))
			call writefile(s:MRU_starfile, s:MRU_starfile_url)
		elseif getline('.') =~# '^\*'
			call setline('.', substitute(getline('.'), '^\*', ' ', 'g'))
			call remove(s:MRU_starfile, line('.')-1)
			call writefile(s:MRU_starfile, s:MRU_starfile_url)
		endif
		setlocal nomodifiable
		return
	...
	call s:MRU_LoadList()
	if match(s:MRU_starfile, a:star) != -1
		return '*'
	else
		return ' '
	endif
	...
	if a:0
		...
		let list = s:MRU_files
	endif

	for star in s:MRU_starfile
		if star == ''
			continue
		endif

		if match(list, star) != -1
			echo 'bingo'
		endif
	endfor
	return
...
command! MruEcho call s:MruEcho()
function! s:MruEcho()
	call s:MRU_LoadList()
	echo s:MRU_files
endfunction
